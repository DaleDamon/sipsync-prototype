/**
 * Script to add Brindille Chicago to the database
 *
 * Usage: node scripts/addBrindille.js
 */
...
const restaurantData = {
  ...
  contact: {
    ...
    website: 'https://www.brindille-chicago.com/'
  }
};

async function addBrindille() {
  console.log('\n========================================');
  console.log('Adding Brindille Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Brindille already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Brindille already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Brindille Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Brindille:', error.message);
    console.error(error);
    process.exit(1);
  }
