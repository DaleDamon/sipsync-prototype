/**
 * Script to add Ambar Chicago to the database
 *
 * Usage: node scripts/addAmbar.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 547-1700',
    website: 'https://ambarrestaurant.com/ambarchicago'
  }
};

async function addAmbar() {
  console.log('\n========================================');
  console.log('Adding Ambar Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Ambar already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Ambar already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Ambar Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Ambar:', error.message);
    console.error(error);
    process.exit(1);
  }
