// One-off script: add Cindy's Rooftop to Firestore
// Run: node scripts/addCindysRooftop.js
require('dotenv').config();
const { db } = require('../firebase');

async function addRestaurant() {
  const name = "Cindy's Rooftop";
  const address = {
  ...
  const latitude = 41.8817;
  const longitude = -87.6247;
