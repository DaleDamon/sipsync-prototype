// One-off script: add Segnatore to Firestore
// Run: node scripts/addSegnatore.js
require('dotenv').config();
const { db } = require('../firebase');

async function addRestaurant() {
  const name = 'Segnatore';
  const address = {
    street: '1001 N California Ave',
    city: 'Chicago',
    state: 'IL',
    zipCode: '60622',
    country: 'USA',
  };
  const latitude = 41.8994236;
  const longitude = -87.6964761;
