// One-off script: add The Alston to Firestore
// Run: node scripts/addTheAlston.js
require('dotenv').config();
const { db } = require('../firebase');

async function addRestaurant() {
  const name = 'The Alston';
  const address = {
    street: '750 N State St',
    city: 'Chicago',
    state: 'IL',
    zipCode: '60654',
    country: 'USA',
  };
  const latitude = 41.8962;
  const longitude = -87.6286;
