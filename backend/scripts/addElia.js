/**
 * Script to add Elia Chicago to the database
 *
 * Usage: node scripts/addElia.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (773) 770-3658',
    website: 'https://eliachicago.com/'
  }
};

async function addElia() {
  console.log('\n========================================');
  console.log('Adding Elia Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Elia already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Elia already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Elia Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Elia:', error.message);
    console.error(error);
    process.exit(1);
  }
