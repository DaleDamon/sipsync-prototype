/**
 * Script to add HIDE+SEEK Chicago to the database
 *
 * Usage: node scripts/addHideSeek.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 680-8217',
    website: 'https://www.hideandseekchicago.com/'
  }
};

async function addHideSeek() {
  console.log('\n========================================');
  console.log('Adding HIDE+SEEK Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if HIDE+SEEK already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ HIDE+SEEK already exists in the database');
    ...
    console.log('\n========================================');
    console.log('HIDE+SEEK Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding HIDE+SEEK:', error.message);
    console.error(error);
    process.exit(1);
  }
