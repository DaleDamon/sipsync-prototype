/**
 * Script to add Kinzie Chophouse Chicago to the database
 *
 * Usage: node scripts/addKinzieChophouse.js
 */
...
const restaurantData = {
  name: 'Kinzie Chophouse',
  address: {
    street: '400 N Wells St',
    city: 'Chicago',
    state: 'IL',
    zipCode: '60654',
    country: 'USA',
  ...
  contact: {
    phone: '+1 (312) 822-0191',
    website: 'https://kinziechophouse.com/'
  }
};

async function addKinzieChophouse() {
  console.log('\n========================================');
  console.log('Adding Kinzie Chophouse Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Kinzie Chophouse already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Kinzie Chophouse already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Kinzie Chophouse Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Kinzie Chophouse:', error.message);
    console.error(error);
    process.exit(1);
  }
