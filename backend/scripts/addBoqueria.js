/**
 * Script to add Boqueria Chicago to the database
 *
 * Usage: node scripts/addBoqueria.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 257-3177',
    website: 'https://boqueriarestaurant.com/location/chi-fulton-market/'
  }
};

async function addBoqueria() {
  console.log('\n========================================');
  console.log('Adding Boqueria Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Boqueria already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Boqueria already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Boqueria Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Boqueria:', error.message);
    console.error(error);
    process.exit(1);
  }
