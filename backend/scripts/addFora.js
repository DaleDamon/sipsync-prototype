/**
 * Script to add Fora Chicago to the database
 *
 * Usage: node scripts/addFora.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 764-1933',
    website: 'https://forachicago.com/'
  }
};

async function addFora() {
  console.log('\n========================================');
  console.log('Adding Fora Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Fora already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Fora already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Fora Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Fora:', error.message);
    console.error(error);
    process.exit(1);
  }
