/**
 * Script to add Carmine's Chicago to the database
 *
 * Usage: node scripts/addCarmines.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 988-7676',
    website: 'https://carminesitalian.com/chicago/home/'
  }
};

async function addCarmines() {
  console.log('\n========================================');
  console.log("Adding Carmine's Chicago to Database");
  console.log('========================================\n');

  try {
    // Check if Carmine's already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log("⚠ Carmine's already exists in the database");
    ...
    console.log('\n========================================');
    console.log("Carmine's Added Successfully!");
    console.log('========================================\n');

  } catch (error) {
    console.error("✗ Error adding Carmine's:", error.message);
    console.error(error);
    process.exit(1);
  }
