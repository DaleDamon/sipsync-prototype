/**
 * Script to add The Albert Chicago to the database
 *
 * Usage: node scripts/addTheAlbert.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 471-3883',
    website: 'https://thealbertchicago.com'
  }
};

async function addTheAlbert() {
  console.log('\n========================================');
  console.log('Adding The Albert Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if The Albert already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ The Albert already exists in the database');
    ...
    console.log('\n========================================');
    console.log('The Albert Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding The Albert:', error.message);
    console.error(error);
    process.exit(1);
  }
