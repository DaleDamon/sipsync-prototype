/**
 * Script to add Avec River North to the database
 *
 * Usage: node scripts/addAvec.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 736-1778',
    website: 'https://www.avecrestaurant.com/location/avec-river-north/'
  }
};

async function addAvec() {
  console.log('\n========================================');
  console.log('Adding Avec River North to Database');
  console.log('========================================\n');

  try {
    // Check if Avec already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Avec River North already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Avec River North Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Avec River North:', error.message);
    console.error(error);
    process.exit(1);
  }
