/**
 * Script to add Bar La Rue Chicago to the database
 *
 * Usage: node scripts/addBarLaRue.js
 */
...
const restaurantData = {
  ...
  contact: {
    ...
    website: 'https://www.barlarue.com/'
  }
};

async function addBarLaRue() {
  console.log('\n========================================');
  console.log('Adding Bar La Rue Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Bar La Rue already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Bar La Rue already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Bar La Rue Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Bar La Rue:', error.message);
    console.error(error);
    process.exit(1);
  }
