/**
 * Script to add Pizzeria Portofino to the database
 *
 * Usage: node scripts/addPizzeriaPortofino.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 900-9018',
    website: 'https://www.pizzeriaportofino.com'
  }
};

async function addPizzeriaPortofino() {
  console.log('\n========================================');
  console.log('Adding Pizzeria Portofino to Database');
  console.log('========================================\n');

  try {
    // Check if Pizzeria Portofino already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Pizzeria Portofino already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Pizzeria Portofino Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Pizzeria Portofino:', error.message);
    console.error(error);
    process.exit(1);
  }
