/**
 * Script to add Wildfire Chicago to the database
 *
 * Usage: node scripts/addWildfire.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 787-9000',
    website: 'https://wildfirerestaurant.com/chicago'
  }
};

async function addWildfire() {
  console.log('\n========================================');
  console.log('Adding Wildfire Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Wildfire already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Wildfire already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Wildfire Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Wildfire:', error.message);
    console.error(error);
    process.exit(1);
  }
