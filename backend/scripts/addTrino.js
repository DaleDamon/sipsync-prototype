/**
 * Script to add Trino Chicago to the database
 *
 * Usage: node scripts/addTrino.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 446-9043',
    website: 'https://trinochicago.com'
  }
};

async function addTrino() {
  console.log('\n========================================');
  console.log('Adding Trino Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Trino already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Trino already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Trino Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Trino:', error.message);
    console.error(error);
    process.exit(1);
  }
