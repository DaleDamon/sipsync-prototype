// One-off script: add Hugo's Frog Bar & Fish House to Firestore
// Run: node scripts/addHugosFrogBar.js
require('dotenv').config();
const { db } = require('../firebase');

async function addRestaurant() {
  const name = "Hugo's Frog Bar & Fish House";
  const address = {
    street: '1024 N Rush St',
    city: 'Chicago',
    state: 'IL',
    zipCode: '60611',
    country: 'USA',
  };
  const latitude = 41.9005;
  const longitude = -87.6258;
