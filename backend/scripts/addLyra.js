/**
 * Script to add LÝRA Chicago to the database
 *
 * Usage: node scripts/addLyra.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 660-7722',
    website: 'https://www.lyrarestaurant.com/'
  }
};

async function addLyra() {
  console.log('\n========================================');
  console.log('Adding LÝRA Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if LÝRA already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ LÝRA already exists in the database');
    ...
    console.log('\n========================================');
    console.log('LÝRA Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding LÝRA:', error.message);
    console.error(error);
    process.exit(1);
  }
