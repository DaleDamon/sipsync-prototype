/**
 * Script to add TAO Chicago to the database
 *
 * Usage: node scripts/addTao.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (224) 888-0388',
    website: 'https://taogroup.com/venues/tao-asian-bistro-chicago/'
  }
};

async function addTao() {
  console.log('\n========================================');
  console.log('Adding TAO Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if TAO Chicago already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ TAO Chicago already exists in the database');
    ...
    console.log('\n========================================');
    console.log('TAO Chicago Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding TAO Chicago:', error.message);
    console.error(error);
    process.exit(1);
  }
