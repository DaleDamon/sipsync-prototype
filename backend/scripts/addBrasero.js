/**
 * Script to add Brasero Chicago to the database
 *
 * Usage: node scripts/addBrasero.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (872) 342-2079',
    website: 'https://www.braserochicago.com/'
  }
};

async function addBrasero() {
  console.log('\n========================================');
  console.log('Adding Brasero Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Brasero already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Brasero already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Brasero Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Brasero:', error.message);
    console.error(error);
    process.exit(1);
  }
