/**
 * Script to add Rosebud on Rush Chicago to the database
 *
 * Usage: node scripts/addRosebudOnRush.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 266-6444',
    website: 'https://rosebudrestaurants.com/our-locations/rosebud-on-rush/'
  }
};

async function addRosebudOnRush() {
  console.log('\n========================================');
  console.log('Adding Rosebud on Rush Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Rosebud on Rush already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Rosebud on Rush already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Rosebud on Rush Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Rosebud on Rush:', error.message);
    console.error(error);
    process.exit(1);
  }
