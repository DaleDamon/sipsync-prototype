/**
 * Script to add Petite Edith Chicago to the database
 *
 * Usage: node scripts/addPetiteEdith.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 210-0623',
    website: 'https://www.petiteedith.com/'
  }
};

async function addPetiteEdith() {
  console.log('\n========================================');
  console.log('Adding Petite Edith Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Petite Edith already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Petite Edith already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Petite Edith Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Petite Edith:', error.message);
    console.error(error);
    process.exit(1);
  }
