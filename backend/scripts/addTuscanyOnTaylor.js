/**
 * Script to add Tuscany on Taylor Chicago to the database
 *
 * Usage: node scripts/addTuscanyOnTaylor.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 829-1990',
    website: 'https://www.tuscanychicago.com/taylor'
  }
};

async function addTuscanyOnTaylor() {
  console.log('\n========================================');
  console.log('Adding Tuscany on Taylor Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Tuscany on Taylor already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Tuscany on Taylor already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Tuscany on Taylor Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Tuscany on Taylor:', error.message);
    console.error(error);
    process.exit(1);
  }
