// One-off script: add The Village to Firestore
// Run: node scripts/addTheVillage.js
require('dotenv').config();
const { db } = require('../firebase');

async function addRestaurant() {
  const name = 'The Village';
  const address = {
    street: '71 W Monroe St',
    city: 'Chicago',
    state: 'IL',
    zipCode: '60603',
    country: 'USA',
  };
  const latitude = 41.8804;
  const longitude = -87.6302;
