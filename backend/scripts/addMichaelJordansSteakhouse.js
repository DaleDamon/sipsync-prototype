/**
 * Script to add Michael Jordan's Steak House Chicago to the database
 *
 * Usage: node scripts/addMichaelJordansSteakhouse.js
 */
...
const restaurantData = {
  name: "Michael Jordan's Steak House",
  address: {
  ...
  contact: {
    phone: '+1 (312) 321-8823',
    website: 'https://www.michaeljordansteakhouse.com/chicago'
  }
};

async function addMichaelJordansSteakhouse() {
  console.log('\n========================================');
  console.log("Adding Michael Jordan's Steak House Chicago to Database");
  console.log('========================================\n');

  try {
    // Check if Michael Jordan's Steak House already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log("⚠ Michael Jordan's Steak House already exists in the database");
    ...
    console.log('\n========================================');
    console.log("Michael Jordan's Steak House Added Successfully!");
    console.log('========================================\n');

  } catch (error) {
    console.error("✗ Error adding Michael Jordan's Steak House:", error.message);
    console.error(error);
    process.exit(1);
  }
