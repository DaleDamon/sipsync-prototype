/**
 * Script to add Bocaditos Chicago to the database
 *
 * Usage: node scripts/addBocaditos.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 526-3306',
    website: 'https://www.bocaditoschicago.com/'
  }
};

async function addBocaditos() {
  console.log('\n========================================');
  console.log('Adding Bocaditos Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Bocaditos already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Bocaditos already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Bocaditos Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Bocaditos:', error.message);
    console.error(error);
    process.exit(1);
  }
