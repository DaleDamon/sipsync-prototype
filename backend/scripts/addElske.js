/**
 * Script to add Elske Chicago to the database
 *
 * Usage: node scripts/addElske.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 733-1314',
    website: 'https://elskerestaurant.com/'
  }
};

async function addElske() {
  console.log('\n========================================');
  console.log('Adding Elske Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Elske already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Elske already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Elske Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Elske:', error.message);
    console.error(error);
    process.exit(1);
  }
