/**
 * Script to add The Publican Chicago to the database
 *
 * Usage: node scripts/addPublican.js
 */
...
async function addPublican() {
  console.log('\n========================================');
  console.log('Adding The Publican Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if The Publican already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ The Publican already exists in the database');
    ...
    console.log('\n========================================');
    console.log('The Publican Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding The Publican:', error.message);
    console.error(error);
    process.exit(1);
  }
