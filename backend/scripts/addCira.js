/**
 * Script to add Cira Chicago to the database
 *
 * Usage: node scripts/addCira.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 761-1777',
    website: 'https://cirachicago.com'
  }
};

async function addCira() {
  console.log('\n========================================');
  console.log('Adding Cira Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Cira already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Cira already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Cira Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Cira:', error.message);
    console.error(error);
    process.exit(1);
  }
