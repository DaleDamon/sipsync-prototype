/**
 * Script to add Legal Sea Foods Chicago to the database
 *
 * Usage: node scripts/addLegalSeaFoods.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 910-6517',
    website: 'https://www.legalseafoods.com/locations/chicago-marina-city/'
  }
};

async function addLegalSeaFoods() {
  console.log('\n========================================');
  console.log('Adding Legal Sea Foods Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Legal Sea Foods already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Legal Sea Foods already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Legal Sea Foods Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Legal Sea Foods:', error.message);
    console.error(error);
    process.exit(1);
  }
