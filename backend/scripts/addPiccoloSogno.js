/**
 * Script to add Piccolo Sogno Chicago to the database
 *
 * Usage: node scripts/addPiccoloSogno.js
 */
...
async function addPiccoloSogno() {
  console.log('\n========================================');
  console.log('Adding Piccolo Sogno Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Piccolo Sogno already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Piccolo Sogno already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Piccolo Sogno Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Piccolo Sogno:', error.message);
    console.error(error);
    process.exit(1);
  }
