/**
 * Script to add Mart Anthony's Chicago to the database
 *
 * Usage: node scripts/addMartAnthonys.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 421-4292',
    website: 'https://www.martanthonys.com/'
  }
};

async function addMartAnthonys() {
  console.log('\n========================================');
  console.log("Adding Mart Anthony's Chicago to Database");
  console.log('========================================\n');

  try {
    // Check if Mart Anthony's already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log("⚠ Mart Anthony's already exists in the database");
    ...
    console.log('\n========================================');
    console.log("Mart Anthony's Added Successfully!");
    console.log('========================================\n');

  } catch (error) {
    console.error("✗ Error adding Mart Anthony's:", error.message);
    console.error(error);
    process.exit(1);
  }
