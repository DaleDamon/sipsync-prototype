// One-off script: add El Che Steakhouse & Bar to Firestore
// Run: node scripts/addElCheSteakhouse.js
require('dotenv').config();
const { db } = require('../firebase');

async function addRestaurant() {
  const name = 'El Che Steakhouse & Bar';
  const address = {
    street: '845 W Washington Blvd',
    city: 'Chicago',
    state: 'IL',
    zipCode: '60607',
    country: 'USA',
  };
  const latitude = 41.8829397;
  const longitude = -87.6491833;
