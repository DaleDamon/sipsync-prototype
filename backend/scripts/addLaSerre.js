/**
 * Script to add La Serre Chicago to the database
 *
 * Usage: node scripts/addLaSerre.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 500-1128',
    website: 'https://www.dinelaserre.com/'
  }
};

async function addLaSerre() {
  console.log('\n========================================');
  console.log('Adding La Serre Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if La Serre already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ La Serre already exists in the database');
    ...
    console.log('\n========================================');
    console.log('La Serre Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding La Serre:', error.message);
    console.error(error);
    process.exit(1);
  }
