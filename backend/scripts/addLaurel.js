/**
 * Script to add Laurel Chicago to the database
 *
 * Usage: node scripts/addLaurel.js
 */
...
const restaurantData = {
  ...
  contact: {
    phone: '+1 (312) 944-4970',
    website: 'https://www.laurelchicago.com'
  }
};

async function addLaurel() {
  console.log('\n========================================');
  console.log('Adding Laurel Chicago to Database');
  console.log('========================================\n');

  try {
    // Check if Laurel already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Laurel already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Laurel Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Laurel:', error.message);
    console.error(error);
    process.exit(1);
  }
