/**
 * Script to add Gene and Georgetti to the database
 *
 * Usage: node scripts/addGeneGeorgetti.js
 */
...
async function addGeneGeorgetti() {
  console.log('\n========================================');
  console.log('Adding Gene & Georgetti to Database');
  console.log('========================================\n');

  try {
    // Check if Gene & Georgetti already exists
    const existingSnapshot = await db.collection('restaurants')
      .where('name', '==', restaurantData.name)
      .get();

    if (!existingSnapshot.empty) {
      console.log('⚠ Gene & Georgetti already exists in the database');
    ...
    console.log('\n========================================');
    console.log('Gene & Georgetti Added Successfully!');
    console.log('========================================\n');

  } catch (error) {
    console.error('✗ Error adding Gene & Georgetti:', error.message);
    console.error(error);
    process.exit(1);
  }
